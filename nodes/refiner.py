...
def refine_response(query: str, rag_output: str) -> str:
    ...
    prompt = (
        ...
        "If the context truly has NO relevant information at all, say "
        "'I could not find specific information about this topic.'\n\n"
        f"Question: {query}\n\n"
        f"Context:\n{rag_output[:12000]}\n\n"
        "Answer:"
    )
...
def _build_factual_prompt(
    ...
    return (
        "You are a strict factual editor and an engaging technical tutor.\n\n"
        "Your task is to fix the user's generated answer based ONLY on the "
        "provided evidence, and then engage the user.\n\n"
        f"Here is the Original Query: {query}\n"
        f"Here is the Evidence: {rag_output[:12000]}\n"
        f"Here is the Report on which claims are true and false: {claim_report}\n\n"
        "Follow these strict rules:\n"
        "1. Rewrite the final answer to be completely factually accurate based on the evidence.\n"
        "2. Remove any contradicted claims and preserve all supported ones.\n"
        "3. List the corrections you made under a '📝 Corrections:' header.\n"
        "4. End your entire response with a single, engaging follow-up question "
        "related to the topic to test the user's understanding or guide them "
        "to the next logical step.\n\n"
        "Make the follow-up question highly relevant to the original query."
    )
